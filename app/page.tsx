...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#0d0d0d] text-gray-200 font-sans pb-28 selection:bg-blue-500/30">
      ...
      <main className="p-4 max-w-xl mx-auto">
                ...
                return (
                  <div key={s.code} className="bg-[#161616] border border-gray-800 rounded-2xl p-4 hover:border-gray-700 transition-all">
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-gray-500 font-bold uppercase tracking-tighter">{s.lot} LOT • AVG <span className="text-gray-400">{formatRp(s.avg)}</span></span>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-[10px] text-gray-500 font-bold mb-0.5 tracking-tighter uppercase text-[8px]">Market Price</p>
                    ...
                    <div className="flex justify-end mt-2 pt-2 border-t border-gray-800/50">
                      <p className={`text-[11px] font-black ${pl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>P/L: {pl >= 0 ? '▲ +' : '▼ -'}Rp {formatRp(Math.abs(pl))} ({pct.toFixed(2)}%)</p>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="animate-fade-in space-y-4 pt-2">
            ...
            {classSignals.map((sig, i) => (
              <div key={i} className="bg-[#161616] border-l-4 border-l-blue-500 border-y border-r border-gray-800 rounded-r-2xl p-5 shadow-xl">
                ...
                <p className="text-xs text-gray-400 leading-relaxed mb-4">{sig.desc}</p>
        ...
        {activeTab === "global" && (
          <div className="animate-fade-in space-y-6 pt-2 text-center">
             <h2 className="text-sm font-black text-gray-500 uppercase tracking-[0.4em] mb-6">Global Macro Charts</h2>
             <div className="bg-[#111] border border-gray-800 rounded-3xl p-4 overflow-hidden shadow-xl mb-4">
                <iframe src="https://s.tradingview.com/widgetembed/?symbol=COINBASE:BTCUSD&interval=D&theme=dark" width="100%" height="250" frameBorder="0"></iframe>
             </div>
             <div className="bg-[#111] border border-gray-800 rounded-3xl p-4 overflow-hidden shadow-xl">
                <iframe src="https://s.tradingview.com/widgetembed/?symbol=OANDA:XAUUSD&interval=D&theme=dark" width="100%" height="250" frameBorder="0"></iframe>
             </div>
          </div>
        )}

        {/* TAB DONATE: QRIS SECTION */}
        {activeTab === "donate" && (
          <div className="animate-fade-in pt-4 max-w-sm mx-auto">
            <div className="bg-[#1a1a1a] border border-gray-800 rounded-[2.5rem] p-8 shadow-2xl text-center relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-500 via-magenta-500 to-cyan-500"></div>
              
              <h2 className="text-xl font-black text-white uppercase tracking-widest mb-4">
                {donationConfig.title}
              </h2>
              
              <p className="text-[11px] text-gray-500 leading-relaxed mb-8 font-medium italic">
                "{donationConfig.description}"
              </p>

              {/* QRIS FRAME */}
              <div className="bg-white p-4 rounded-3xl inline-block shadow-[0_0_30px_rgba(255,255,255,0.05)] mb-6 border-4 border-gray-800">
                <img 
                  src={donationConfig.qrisUrl} 
                  alt="QRIS Donasi" 
                  className="w-56 h-auto grayscale-[0.2] hover:grayscale-0 transition-all duration-500"
                  onError={(e) => {
                    // Fallback kalau gambar gak ketemu
                    (e.target as any).src = "https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=DonasiServerPediaboy";
                  }}
                />
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black text-gray-400 tracking-[0.3em] uppercase">Scan QRIS via</p>
                <p className="text-xs font-bold text-gray-300">DANA • OVO • GOPAY • SHOPEEPAY • M-BANKING</p>
              </div>
            </div>
          </div>
        )}
      ...
      <nav className="fixed bottom-0 w-full bg-[#0d0d0d]/95 backdrop-blur-2xl border-t border-gray-800 py-4 px-6 flex justify-around items-center z-50">
        <button onClick={() => setActiveTab("home")} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'home' ? 'text-white scale-110' : 'text-gray-600 opacity-50'}`}>
          <span className="text-xl">📊</span><span className="text-[8px] font-black uppercase tracking-widest">Aset</span>
        </button>
        <button onClick={() => setActiveTab("plan")} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'plan' ? 'text-white scale-110' : 'text-gray-600 opacity-50'}`}>
          <span className="text-xl">🎯</span><span className="text-[8px] font-black uppercase tracking-widest">Plan</span>
        </button>
        <button onClick={() => setActiveTab("global")} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'global' ? 'text-white scale-110' : 'text-gray-600 opacity-50'}`}>
          <span className="text-xl">🌐</span><span className="text-[8px] font-black uppercase tracking-widest">Macro</span>
        </button>
        <button onClick={() => setActiveTab("donate")} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === 'info' ? 'text-white scale-110 font-bold' : 'text-gray-600 opacity-50'}`}>
          <span className="text-xl">💸</span><span className="text-[8px] font-black uppercase tracking-widest">Donate</span>
        </button>
      </nav>
    </div>
  );
